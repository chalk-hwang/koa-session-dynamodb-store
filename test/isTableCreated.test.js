...
const RESOURCE_NOT_FOUND_EXCEPTION = Promise.reject({
...
describe('DynamoDBStore/isTableCreated', () => {
    ...
    beforeEach(() => {
        ...
        store.dynamoService.describeTable = jest.fn(() => ({promise: () => Promise.resolve(describeTable)}));
        store.dynamoService.describeTimeToLive = jest.fn(() => ({promise: () => Promise.resolve(describeTimeToLive)}));
    });
    ...
    it('should call describeTable', async () => {
        await store.isTableCreated();
        expect(store.dynamoService.describeTimeToLive.mock.calls).toEqual([[{TableName: TABLE_NAME}]]);
    });
